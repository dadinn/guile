          ...
          ((begin
             ...
             (lambda (content char)
               (when (not (< 0 counter 2))
                 ;; ensure test expression only gets evaluated once for expect process.
...
MATCHED!!!"
  (call-with-output-string
    (lambda (output-port)
      ...
      ;; from lexical scope, overriding internal named parameter value
      (let ((expect-char-proc
...
(test-equal "expect macro captures expect-port and expect-char-proc bindings via named parameters"
  "Some stuff here...
MATCHING_STUFF_2023!
MATCHED!!!"
  (call-with-output-string
...
MATCHING_STUFF_2023!!!"
        (lambda (input-port)
          (parameterize
              ;; expect macro should use module-internal named parameters
              ;; when parameters have no bindings defined in lexical scope
              (((@@ (ice-9 expect) expect-port) input-port)
               ((@@ (ice-9 expect) expect-char-proc)
                (lambda (c) (display c output-port))))
            (expect
             ((begin
                (lambda (content eof?)
...
"
  (let* ((expect-eof-proc
         ...
         (expect-timeout-proc
          (lambda (content)
            (error "TIMEOUT PROC MUST NOT BE CALLED!!!")))
  ...
  (call-with-output-string
    (lambda (output-port)
      (let* ((expect-char-proc
             ...
             (expect-eof-proc
              (lambda (content)
                (error "EOF PROC MUST NOT BE CALLED!!!")))
             (expect-timeout-proc
              (lambda (content)
                (error "TIMEOUT PROC MUST NOT BE CALLED!!!")))
             (expect-timeout 1)
...
(test-error
 "cannot use the same port (default STDIN) for both interact and expect input"
 #t
 (with-input-from-string ""
   (lambda ()
     (let ((interact-output-port
            (%make-void-port OPEN_WRITE)))
       (dynamic-wind
         (const #t)
         (lambda ()
